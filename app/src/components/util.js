export function UtilTableToCSV(tableID) {
...
export function UtilParseDate(string) {
...
export function UtilParseDateTime(string) {
    if (string === "0000-00-00")
        return "";
    //return new Date(string);
    else
...
export function UtilGetSQLDate(date) {
  return `${date.getFullYear()}-${date.getMonth() < 9 ? "0" : ""}${(date.getMonth() + 1)}-${date.getDate() < 10 ? "0" : ""}${date.getDate()}`;
}

export function UtilConvertDateToSQLFormat(date) {
    if (date === "")
        return  "0000-00-00";
    else
    {
